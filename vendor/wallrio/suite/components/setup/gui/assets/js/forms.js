...
// form.focus('[name="component-form-register"]','[name="password"]');
form.focus = function(formId,elemendId){    
    if(!document.querySelector(formId))
        return false;

    var el = document.querySelector(formId).querySelectorAll('input:not([type="submit"])');
...
form.register = function(formId,options,callback,validator){
    if(!document.querySelector(formId))
        return false;
    
    document.querySelector(formId).onsubmit = function(){
        var data = Object();
        var el = this.querySelectorAll('input:not([type="submit"])');
        for (var i = 0; i < el.length; i++) {
            var name = el[i].name;
        ...
        if(validator){
            var resultValidator = validator(data);
        ...
        form.send(data,options,callback,validator);
...
form.send = function(dataSend,options,callback,validator){
