...
// simplifyForm.focus('[name="component-form-register"]','[name="password"]');
...
simplifyForm.change = function(formId,callback){        
    if(!document.querySelector(formId))
        return false;

    var el = document.querySelector(formId).querySelectorAll('input:not([type="submit"]),textarea,select');
    for (var i = 0; i < el.length; i++) {        
        simplifyForm.addEvent(el[i],'keyup',function(e,obj){
            if(callback)
                callback(obj);
        });

        simplifyForm.addEvent(el[i],'change',function(e,obj){
            if(callback)
                callback(obj);
        });
